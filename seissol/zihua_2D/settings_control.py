...
surrogate_model_settings = surrogate_model.SKLearnGPSettings(
    ...
    correlation_kernel=RBF(length_scale=(0.1, 0.1), length_scale_bounds=((1e-2, 1e1),(1e-2, 1e1))),
    data_noise=1e-6,
    ...
    init_seed=0,
    checkpoint_load_file="results_seissol_zihua_2D/surrogate_checkpoint_pretraining.pkl",
    checkpoint_save_path=Path("results_seissol_zihua_2D"),
)
...
control_logger_settings = utils.LoggerSettings(
    do_printing=True,
    logfile_path=Path("results_seissol_zihua_2D/online.log"),
    write_mode="w",
)
